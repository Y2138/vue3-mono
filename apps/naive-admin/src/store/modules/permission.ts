...
export const usePermissionStore = defineStore('permission', () => {
  ...
  async function refreshUserPermissions(userId?: string): Promise<boolean> {
    ...
    try {
      ...
      // 并行获取权限和角色信息
      const [permissionsSuccess, rolesSuccess] = await Promise.all([fetchPermissions(true), fetchRoles(true)])
  ...
  async function hasAnyPermission(permissionCodes: string[]): Promise<boolean> {
    ...
    // 检查本地缓存
    const hasLocalPermission = permissionCodes.some((code) => userPermissionSet.value.has(code))
  ...
  async function hasAllPermissions(permissionCodes: string[]): Promise<boolean> {
    ...
    // 检查本地缓存
    const hasAllLocalPermissions = permissionCodes.every((code) => userPermissionSet.value.has(code))
   ...
   */
  async function checkMultiplePermissions(permissionCodes: string[]): Promise<Record<string, boolean>> {
    const userStore = useUserStore()
    const userId = userStore.userProfile?.id
  ...
  function getUserPermissionDetails(): Permission[] {
    return userPermissions.value.map((code) => getPermissionDetail(code)).filter((permission) => permission !== null) as Permission[]
  }
  ...
  function getUserRoleDetails(): Role[] {
    return userRoles.value.map((code) => getRoleDetail(code)).filter((role) => role !== null) as Role[]
  }
