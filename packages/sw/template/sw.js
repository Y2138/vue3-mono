 ...
 * 预缓存配置
 */
const PRECACHE_CONFIG = { enabled: false };
...
const expirationPlugin = new ExpirationPlugin({
  maxEntries: config.staticCacheMaxEntries,
  maxAgeSeconds: config.staticCacheMaxAgeDay * 24 * 60 * 60, // 天数转秒数
...
registerRoute(
  ...
  new CacheFirst({
    cacheName: `${config.cachePrefix}-static-v1`,
    plugins: [expirationPlugin],
  })
);

// 设置CDN资源缓存策略
if (config.cdnDomains.length > 0) {
  const cdnRoute = new Route(
    ({ url }) => {
      return config.cdnDomains.some(domain => url.hostname.includes(domain));
    },
    new CacheFirst({
      cacheName: `${config.cachePrefix}-cdn-v1`,
      plugins: [expirationPlugin],
    })
  );
