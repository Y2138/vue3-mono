...
export class MonitoringService {
  ...
  private initializeMetrics(): void {
    const services = ['UserService', 'Health', 'PrismaService']
  ...
  getErrorStats(): Record<string, any> {
    ...
    // 获取最近的错误请求
    stats.recentErrors = this.recentRequests.filter((req) => !req.success).slice(-20)
  ...
  private calculateRequestsPerSecond(serviceName: string): number {
    ...
    const recentRequests = this.recentRequests.filter((req) => this.extractServiceName(req.method) === serviceName && req.endTime >= oneMinuteAgo)
