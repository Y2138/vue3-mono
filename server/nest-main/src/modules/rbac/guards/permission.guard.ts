...
export class PermissionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // 获取需要的权限
    const requiredPermissions = this.reflector.get<string[]>('permissions', context.getHandler()) || this.reflector.get<string[]>('permissions', context.getClass())
    ...
    if (!hasPermission) {
      this.logger.warn(`User ${user.phone} denied access - Required: [${requiredPermissions.join(', ')}]`)
  ...
  private isSuperAdmin(user: User & { roles?: any[] }): boolean {
    return (user.roles && user.roles.some((role) => role.name === '超级管理员' || role.name === 'super_admin')) || false
  }
